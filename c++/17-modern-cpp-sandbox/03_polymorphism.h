...
TEST(PolymorphismTest, CompareMemoryLayout) {
    ...
    /*
        That’s a classic Empty Base Optimization (EBO) failure in the
       IntelliSense engine.
        ...
        Why this happened (Technical Reason)
            ...
            Real GCC: Sees Deleter is empty and uses EBO to make it occupy 0
       bytes. Total = 8 bytes.

            IntelliSense: Sometimes fails the EBO check on specific Standard
       Library implementations (like your GCC 15.2 headers), assigning 8 bytes
       to the pointer + padding/size for the "empty" deleter. Total = 16 bytes.
    */
    if constexpr (sizeof(void*) == 8) {
        static_assert(sizeof(shapes[0]) == 8,
                      "This only fails in VS Code's broken IntelliSense");
    }
#endif
    // std::println("Actual size of shapes[0] (unique_ptr<Shape>): {}",
    // sizeof(shapes[0]));

    static_assert(sizeof(int) == 4, "Expected int size to be 4 bytes");
    // The pointer vector stores 8-byte addresses. The flat vector stores 4-byte
    // ints.
    EXPECT_EQ(sizeof(shapes[0]), sizeof(void*)); // Size of unique_ptr<Shape>
    EXPECT_EQ(sizeof(flat_circles[0]),
              sizeof(int)); // Size of FlatCircle (4-byte int)

    // TEST 2: Logic Check
    int sum_poly = 0;
    for (const auto& s : shapes)
        sum_poly += s->getValue(); // VTable lookup + Pointer hop

    int sum_flat = 0;
    for (const auto& c : flat_circles)
        sum_flat += c.getValue(); // Direct call + Cache friendly
