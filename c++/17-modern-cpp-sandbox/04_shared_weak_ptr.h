...
// The weak reference counter is illustrative only and not a true std::weak_ptr
// lifetime counter.
...
class Tile final {
  public:
    ...
    ~Tile() noexcept {
        // std::println("Destroying Tile(type={}, weak_count={})",
        // to_string(type), weak_count);
    }

    // Demonstration-only weak reference tracking.
    // This count is incremented on requests, not by actual std::weak_ptr
    // lifetime. A proper solution would wrap std::weak_ptr and update counts
    // when objects copy/move/destroy.
...
class Registry final {
  public:
    ...
    [[nodiscard]] std::weak_ptr<Tile> get_tile(size_t x, size_t y) {
        ...
        // Just a simple way to assign types
        auto type = static_cast<Tile::Type>((x + y) % 3);
...
// A Layer holds weak references to tiles and releases them when the layer is
// destroyed.
class Layer final {
  public:
...
TEST(SharedPtrTest, RealUse) {
    ...
    EXPECT_EQ(tile.lock()->get_weak_count(), 4u)
        << "Three layers (+get_tile) have requested the tile";
    EXPECT_EQ(tile.use_count(), 1)
        << "Only the registry holds a shared_ptr to the tile";

    weather.reset();
    nav.reset();
    EXPECT_EQ(tile.lock()->get_weak_count(), 2u)
        << "Only the terrain layer and get_tile";
    EXPECT_EQ(tile.use_count(), 1)
        << "Only the registry holds a shared_ptr to the tile";
...
TEST(MapEditorTest, SharedOwnershipLogic) {
...

    {
        ...
        // We can verify this by getting a temporary shared_ptr from the
        // registry.
